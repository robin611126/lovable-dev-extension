// ============================================================
// lovable dev - Side Panel Templates (Static/HTML)
...
function spTemplateLicenseGate() {
  return '<div class="sp-license-gate">' +
    ...
    '<p class="sp-gate-desc">Enter your license key to activate. Paste the key you received from WhatsApp support or your reseller.</p>' +
...
function spTemplateChannelGate() {
  return '<div class="sp-license-gate sp-channel-gate">' +
    ...
    '<p class="sp-gate-desc">Contact us on WhatsApp to unlock and activate all features instantly.</p>' +
    '<button class="sp-btn-primary" id="sp-join-channel-btn" style="background: linear-gradient(135deg, #25d366, #128c7e); margin-top: 15px; display: flex; align-items: center; justify-content: center; gap: 8px; box-shadow: 0 4px 16px rgba(37, 211, 102, 0.3);">' +
      '👉 Chat on WhatsApp & Unlock' +
    '</button>' +
...
function spTemplateStatusBadge(status, durationType) {
  var tenure = 'Unlimited Access';
  if (durationType === '1_hour') tenure = '1 Hour Trial';
  else if (durationType === '1_week') tenure = '1 Week';
  else if (durationType === '1_month') tenure = '1 Month';
  else if (durationType === 'lifetime') tenure = 'Lifetime';

  if (status === 'trial' || durationType === '1_hour') {
    return '<span class="sp-status-badge sp-badge-test">' + tenure + '</span>';
  }
  return '<span class="sp-status-badge sp-badge-pro">' + tenure + '</span>';
}
