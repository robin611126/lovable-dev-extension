// ============================================
// lovable dev – HTML Templates (content)
...
function templateChannelGate(minimized) {
  return '<div id="ql-header">' +
    '<div class="ql-header-left">' +
      ...
      '<img class="ql-title-logo" src="' + chrome.runtime.getURL('assets/logo-master-lovable-square.png') + '" alt=""><span class="ql-title">lovable dev</span>' +
    '</div>' +
  ...
  '<div id="ql-body">' +
    '<div class="ql-license-gate ql-channel-gate">' +
      ...
      '<p class="ql-gate-desc">Contact us on WhatsApp to get access to all features instantly.</p>' +
      '<button id="ql-join-channel-btn" style="width: 100%; padding: 12px; border: none; border-radius: var(--ql-radius-sm); background: linear-gradient(135deg, #25D366, #128C7E); color: #fff; font-size: 14px; font-weight: 700; cursor: pointer; transition: all var(--ql-transition); margin-top: 15px; display: flex; align-items: center; justify-content: center; gap: 8px; box-shadow: 0 4px 16px rgba(37, 211, 102, 0.3);">' +
        '👉 Contact WhatsApp & Unlock' +
      '</button>' +
...
function templateLicenseGate(minimized) {
  ...
  '<div id="ql-body">' +
    '<div class="ql-license-gate">' +
      ...
      '<p class="ql-gate-desc">Enter your license key to activate. Paste the key you received from WhatsApp support or your reseller.</p>' +
      '<div class="ql-field">' +
      ...
      '<div class="ql-gate-divider"><span>WhatsApp support</span></div>' +
      '<a href="' + QL_DISCORD_SUPPORT + '" target="_blank" rel="noopener noreferrer" class="ql-buy-btn" style="display:block;text-align:center;text-decoration:none">🔑 Request your key via WhatsApp</a>' +
    '</div>' +
  '</div>' +
  '<div id="ql-resize-handle" class="ql-resize-handle"></div>';
}
...
function templateMainUI(greeting, statusBadge, minimized) {
  return '<div id="ql-header">' +
    '<div class="ql-header-left">' +
      '<span class="ql-brand"><img class="ql-brand-logo" src="' + chrome.runtime.getURL('assets/logo-master-lovable-square.png') + '" alt=""><span>lovable dev</span></span>' +
    '</div>' +
    '<div class="ql-header-right">' +
  ...
  '<div id="ql-footer" class="ql-footer">' +
    ...
    '</div>' +
    '<span class="ql-badge-mz">lovable dev</span>' +
  '</div>' +
  '<div id="ql-resize-handle" class="ql-resize-handle"></div>' +
  '<!-- Notifications Panel -->' +
...
function templateExpiredOverlay() {
  return '<div class="ql-sweetalert-box">' +
    ...
    '<p class="ql-sweetalert-text">Your license has expired. Contact WhatsApp support to renew it.</p>' +
    '<div class="ql-sweetalert-actions">' +
      '<button class="ql-sweetalert-btn ql-sweetalert-btn-primary" id="ql-sweetalert-renew">🌐 Contact WhatsApp</button>' +
  ...
  return `
    <div id="ql-whatsapp-overlay" class="ql-overlay" style="display:flex; align-items:center; justify-content:center; position:fixed; top:0; left:0; width:100%; height:100%; background:rgba(0,0,0,0.7); z-index:1000000; font-family:sans-serif;">
      <div style="background:white; padding:30px; border-radius:15px; max-width:400px; text-align:center; position:relative; box-shadow:0 10px 25px rgba(0,0,0,0.2);">
        ...
        <h2 style="margin:0 0 10px; color:#0088cc;">Join Our Community!</h2>
        <p style="color:#555; margin-bottom:20px; line-height:1.5;">Stay updated with the latest features, tips, and exclusive updates. Contact us on WhatsApp for support!</p>
        <a href="https://wa.me/918469279409" target="_blank" id="ql-wa-join" style="display:inline-block; background:#25D366; color:white; padding:12px 25px; border-radius:25px; text-decoration:none; font-weight:bold; transition:background 0.3s;">Contact WhatsApp Support</a>
...
var QL_BRL_PLANS = [
  { name: "Weekly",  price: "49,90",  period: "per week",      popular: false, badge: "7d",  icon: "⚡",
    features: ["Full extension access", "Plan Mode workflow", "Support via WhatsApp"] },
...
function templateBrlCard(plan, idx) {
  ...
  return '<div class="ql-pkg-card ql-pkg-brl' + (plan.popular ? ' ql-pkg-highlight' : '') + '" data-brl-idx="' + idx + '">' +
    ...
    '<button class="ql-pkg-select-btn ql-brl-buy">💬 Contact WhatsApp</button>' +
  '</div>';
}
function templateBrlSection() {
  var cards = QL_BRL_PLANS.map(function(p, i){ return templateBrlCard(p, i); }).join('');
  return '<div class="ql-pay-divider"><span>WhatsApp support</span></div>' +
...
function templatePaymentUI(minimized) {
  ...
  '<div id="ql-body">' +
    '<div class="ql-pay-section">' +
      '<div class="ql-pay-title">Contact WhatsApp Support</div>' +
      templateBrlSection() +
      '<div class="ql-pay-divider"><span>WhatsApp support</span></div>' +
      '<div id="ql-packages-list" class="ql-packages-list">' +
        '<div class="ql-pay-loading">⏳ Contact WhatsApp Support</div>' +
      '</div>' +
    '</div>' +
  '</div>' +
  '<div id="ql-resize-handle" class="ql-resize-handle"></div>';
...
function templatePackageCard(pkg) {
  ...
  return '<div class="ql-pkg-card' + (pkg.is_popular ? ' ql-pkg-highlight' : '') + '" data-pkg-id="' + escapeHtml(pkg.id) + '" data-pkg-name="' + escapeHtml(pkg.name) + '" data-pkg-price="' + '' + '">' +
    ...
    '<ul class="ql-pkg-features">' + features + '</ul>' +
    '<button class="ql-pkg-select-btn">Contact WhatsApp</button>' +
  '</div>';
}
...
function templateCheckoutScreen(pkg, minimized) {
  return '<div id="ql-header">' +
    '<div class="ql-header-left">' +
      '<span class="ql-brand"><img class="ql-brand-logo" src="' + chrome.runtime.getURL('assets/logo-master-lovable-square.png') + '" alt=""><span>WhatsApp Support</span></span>' +
    '</div>' +
    '<div class="ql-header-right">' +
  ...
  '<div id="ql-body">' +
    '<div class="ql-pay-section" style="text-align:center">' +
      '<div class="ql-pay-title">Request your license key on WhatsApp</div>' +
      '<div class="ql-selected-pkg">📦 <strong>' + escapeHtml(pkg.name || 'Selected plan') + '</strong></div>' +
      '<p style="color:var(--ql-text-secondary);font-size:12px;line-height:1.5;margin:10px 0 16px">Support and license purchases are handled through WhatsApp.</p>' +
      '<a href="' + QL_DISCORD_SUPPORT + '" target="_blank" rel="noopener noreferrer" class="ql-confirm-pay-btn" style="display:block;text-decoration:none;margin-bottom:10px">💬 Contact WhatsApp Support</a>' +
      '<button id="ql-confirm-pay" class="ql-buy-btn" style="font-size:12px;width:100%">Contact WhatsApp</button>' +
