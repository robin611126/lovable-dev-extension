// ============================================================
// lovable dev - Side Panel Logic (Business Logic Only)
...
(function(){
  ...
  // Integrity check disabled - authentication removed
  ...
  const LICENSING_BASE = typeof LICENSING_API_BASE !== "undefined" ? LICENSING_API_BASE : "http://localhost:3000";
  const VALIDATE_URL = LICENSING_BASE + "/index.php?route=api/activate";
  ...
  function ensureInternalSessionLocal() {
    ...
    return new Promise(function(resolve) {
      chrome.storage.local.get(["ql_license_valid", "ql_session_id", "ql_user_name", "ql_license_key", "ql_last_heartbeat", "ql_offline_grace_hours", "ql_license_status"], function(res) {
        if (res.ql_license_valid && res.ql_session_id) {
          // Check Offline Grace Period
          const lastHeartbeat = res.ql_last_heartbeat || Date.now();
          const graceHours = res.ql_offline_grace_hours || 48;
          const hoursOffline = (Date.now() - lastHeartbeat) / (1000 * 60 * 60);

          if (hoursOffline > graceHours) {
            chrome.storage.local.set({ ql_license_valid: false, ql_license_status: 'offline_locked' });
            return resolve(); // Will fail downstream due to valid=false
          }

          if (['suspended', 'deleted', 'blocked', 'expired', 'offline_locked'].includes(res.ql_license_status)) {
            // State does not permit action
            return resolve();
          }
  ...
  let sessionId = null, userName = null, expiresAt = null, licenseStatus = null, validityMinutes = null, spActivatedAt = null, heartbeatInterval = null, deviceId = null, isResellerUser = false, durationType = null;
  ...
  chrome.runtime.onMessage.addListener((msg) => {
    if (msg.action === "heartbeat_sync" && msg.data) {
      if (msg.data.duration_type) durationType = msg.data.duration_type;
      _applyLicenseState(msg.data.status, msg.data.valid);
    }
  });

  // Also poll chrome.storage every 30 seconds (backup in case message missed)
  setInterval(function() {
    chrome.storage.local.get(["ql_license_valid", "ql_license_status", "ql_duration_type"], function(res) {
      if (res.ql_duration_type) durationType = res.ql_duration_type;
      _applyLicenseState(res.ql_license_status, res.ql_license_valid);
    });
  }, 30000);

  function _applyLicenseState(status, valid) {
    // Only act if extension is currently showing main UI (user is logged in)
    const body = document.getElementById('sp-body');
    if (!body) return;
    // Don't show lock if already on license gate
    if (body.querySelector('.sp-license-gate') || body.querySelector('#sp-license-form')) return;

    if (valid === false) {
      // Set ql_license_valid=false explicitly (NOT remove) so background gate check works
      chrome.storage.local.set({ ql_license_valid: false, ql_license_status: status || 'invalid' });
      showLicenseGate();
      // Show reason message
      requestAnimationFrame(function() {
        ...
        if (status === 'deleted' || status === 'blocked') {
          log.textContent = '⛔ License Deleted. Please enter a valid license key.';
        } else if (status === 'suspended') {
          log.textContent = '⏸ License Suspended by admin. Contact support.';
        } else if (status === 'expired') {
          log.textContent = '⚠ License Expired. Please renew your subscription.';
        } else {
          log.textContent = '⚠ License Invalid. Please enter a valid key.';
        }
      });
    }
  }
  ...
  async function validateLicense() {
    ...
    try {
      ...
      if(data.valid) {
        ...
        // Play activation success arpeggio
        if (typeof QLSounds !== "undefined" && typeof QLSounds.activation === "function") {
          try { QLSounds.activation(); } catch(sErr) {}
        }
        ...
        chrome.storage.local.set(Object.assign({
          ...
          showAlert('Activated Successfully! 🎉', data.message || 'Your subscription is now active.');
          setTimeout(() => { showMainUI(); startHeartbeat(key); }, 1500);
        });
      } else {
        log.className = 'sp-log sp-log-error'; log.textContent = '✗ ' + spUserText(data.message || 'Invalid response format') + ' (Raw: ' + JSON.stringify(data) + ')';
      }
    } catch(err) {
      log.className = 'sp-log sp-log-error';
  ...
  function showMainUI() {
    ...
    const statusBadge = spTemplateStatusBadge(licenseStatus, durationType);
    const body = document.getElementById('sp-body');
    loadChatHistory(function() {
      const actDateStr = spActivatedAt ? new Date(spActivatedAt).toLocaleString() : 'N/A';
      const expDateStr = expiresAt ? new Date(expiresAt).toLocaleString() : 'Never (Lifetime)';
      body.innerHTML = '<div id="sp-announcement-banner" style="display:none;background:#ff9800;color:#fff;padding:8px;text-align:center;font-size:13px;font-weight:bold;"></div>' +
        ...
        '<div class="sp-profile-card">' +
          ...
          '<div class="sp-license-dates" style="margin-top:8px;font-size:11px;color:var(--ql-text-secondary);display:flex;flex-direction:column;gap:3px;border-top:1px solid rgba(255,255,255,0.08);padding-top:8px;">' +
            '<div>📅 Activated: ' + actDateStr + '</div>' +
            '<div>⏳ Expiry: ' + expDateStr + '</div>' +
          '</div>' +
          '<div class="sp-trial-countdown" id="sp-countdown" style="display:none;margin-top:8px;color:#ff9800;font-weight:bold;"></div>' +
        '</div>' +
        '<div id="sp-reseller-btn" style="display:none;margin-bottom:14px">' +
          '<a href="' + ((typeof DISCORD_SUPPORT_URL !== "undefined" && DISCORD_SUPPORT_URL) || "https://wa.me/918469279409") + '" target="_blank" rel="noopener noreferrer" class="pk-discord-cta">' +
            '🔑 Contact WhatsApp Support<span style="margin-left:auto;font-size:10px;opacity:0.6">→</span>' +
          '</a>' +
        '</div>' +
  ...
  function startHeartbeat(key) {
    ...
    heartbeatInterval = setInterval(async () => {
      try {
        ...
        const data = await bgFetch(LICENSING_BASE + "/index.php?route=api/check", { method: "POST", headers: apiHeaders({ "Content-Type": "application/json" }), body: JSON.stringify({ license_key: key, session_id: sessionId, heartbeat: true, device_id: deviceId }) });
        if(!data.valid) {
    ...
    }, 30000); // 30 seconds check for real-time suspensions
  }
    ...
    chrome.storage.local.get([WA_POPUP_SEEN_KEY], function(res) {
      ...
      overlay.innerHTML =
        '<div class="sp-modal" style="text-align:center">' +
          ...
          '<div class="sp-modal-title" style="color:#0088cc;font-size:16px">Join Our Community!</div>' +
          '<div class="sp-modal-body" style="font-size:12px;color:var(--ql-text-secondary);margin-bottom:16px">' +
            'Stay updated with the latest features, tips, and exclusive updates. Contact us on WhatsApp for support!' +
          '</div>' +
          '<a href="https://wa.me/918469279409" target="_blank" id="sp-wa-join" style="display:inline-block;background:#25D366;color:white;padding:12px 25px;border-radius:25px;text-decoration:none;font-weight:bold;font-size:13px;transition:background 0.3s">Contact WhatsApp Support</a>' +
  ...
  function showChannelGate() {
    ...
    if (joinBtn) {
      joinBtn.addEventListener('click', () => {
        chrome.tabs.create({ url: "https://t.me/+6Ez3pcDd8245O" });
  ...
  async function init() {
    ...
    chrome.storage.local.get(["ql_channel_redirected", "ql_license_valid","ql_license_key","ql_user_name","ql_expires_at","ql_activated_at","ql_license_status","ql_validity_minutes","ql_session_id", "ql_duration_type"], async (res) => {
      ...
      if (res.ql_license_key && res.ql_license_key !== 'BYPASS') {
        try {
          const checkUrl = LICENSING_BASE + "/index.php?route=api/check";
          const data = await bgFetch(checkUrl, {
            method: "POST",
            headers: apiHeaders({ "Content-Type": "application/json" }),
            body: JSON.stringify({
              license_key: res.ql_license_key,
              device_id: deviceId
            })
          });
          ...
          if (data && data.valid) {
            ...
            licenseStatus = data.status || null;
            ...
            // Key has been suspended or invalidated by admin
            await new Promise(resolve => {
              chrome.storage.local.remove(["ql_license_valid","ql_license_key","ql_session_id","ql_user_name","ql_expires_at","ql_activated_at","ql_license_status","ql_validity_minutes","ql_duration_type"], resolve);
            });
            showLicenseGate();
            setTimeout(function() {
              const log = document.getElementById('sp-license-log');
              if (log) {
                log.className = 'sp-log sp-log-error';
                log.textContent = '⛔ ' + (data.message || 'License suspended or invalid.');
              }
            }, 200);
            return;
          }
        } catch (e) {
          console.warn("Verify check failed (offline/network error), falling back to cached state.", e);
        }
      }
      ...
      if (INTERNAL_LICENSE_MODE || res.ql_license_valid) {
        ...
        durationType = res.ql_duration_type || null;
        syncCreditBypassOnLovableTabs(true);
        showMainUI();
