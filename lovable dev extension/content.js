// ============================================
// lovable dev – Business Logic (content)
// HTML templates are in content-templates.js
// ============================================

// Integrity check (safe - no destructive DOM operations)
(function _integrityCheck() {
  // Ensure _pkS stub exists so integrity never triggers
  if (typeof window._pkS === 'undefined') {
    window._pkS = {
      lock: function(){}, check: function(){}, hash: function(s){return s;},
      destroy: function(){}, integrityToken: 'pk_ok', integrityCheck: function(){}
    };
  }
})();


console.log("[ContentScript] lovable dev loaded");
...
// ============================================================
// 30-SECOND LICENSE POLLING in content.js
// Listens for heartbeat_sync from background AND polls storage
// If license deleted/expired - auto-locks the floating UI
// ============================================================
chrome.runtime.onMessage.addListener(function(msg) {
  if (msg.action === "heartbeat_sync" && msg.data) {
    var s = msg.data.status;
    var v = msg.data.valid;
    if (v === false) {
      _contentLicenseLock(s);
    }
  }
});

setInterval(function() {
  chrome.storage.local.get(["ql_license_valid", "ql_license_status"], function(res) {
    var s = res.ql_license_status;
    var v = res.ql_license_valid;
    if (v === false) {
      _contentLicenseLock(s);
    }
  });
...
function _contentLicenseLock(status) {
  var box = document.getElementById("ql-floating");
  if (!box) return;
  // Only lock if currently showing main UI (not already on license gate)
  if (box.querySelector('#ql-validate-btn') || box.querySelector('.ql-license-gate')) return;
  // Set license_valid=false explicitly (NOT remove) so background.js gate check works
  chrome.storage.local.set({ ql_license_valid: false, ql_license_status: status });
  setPkCreditBypass(false);
  if (qlHeartbeatInterval) { clearInterval(qlHeartbeatInterval); qlHeartbeatInterval = null; }
  // Show license gate
  showLicenseGate(box);
  // Show reason message after gate renders
  setTimeout(function() {
    var log = document.getElementById("ql-license-log");
    if (!log) return;
    log.className = "ql-log-error";
    if (status === 'deleted' || status === 'blocked') {
      log.innerText = "⛔ License Deleted. Enter a valid license key.";
    } else if (status === 'expired') {
      log.innerText = "⚠ License Expired. Please renew.";
    } else if (status === 'suspended') {
      log.innerText = "⏸ License Suspended by admin.";
    } else {
      log.innerText = "⚠ License Invalid. Please activate.";
    }
  }, 100);
}
  ...
  setTimeout(() => {
    ...
    if(btn) {
      btn.addEventListener("click", () => {
        chrome.runtime.sendMessage({ action: "openTab", url: "https://t.me/+6Ez3pcDd8245O" });
...
async function validateLicense(){
  ...
  try{
    ...
    if(data.valid){
      ...
      if(log){ log.className = "ql-log-error"; log.innerText = "✗ " + qlUserText(data.message || "Invalid response format") + " (Raw: " + JSON.stringify(data) + ")"; }
    }
  }catch(err){
...
function injectShieldOverlay(){
  ...
  overlay.innerHTML = '<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">' +
    ...
    '</svg>' +
    '<span class="ql-shield-overlay-text">🛡️ Protected by lovable dev</span>' +
...
async function showPaymentUI(box, preselectedPkg){
  ...
  document.querySelectorAll(".ql-brl-buy").forEach(function(btn){
    btn.addEventListener("click", function(){
      ...
      if(!plan) return;
      var msg = "Hello! 👋 I am interested in the *" + plan.name + "* plan for lovable dev (R$ " + plan.price + " - " + plan.period + ").\n\nOpen support for more information.";
      var url = DISCORD_URL;
...
function injectNativeChatOverlay() {
  ...
  if (!document.getElementById("ql-native-badge")) {
    ...
    badge.innerHTML = "⚡ <span>lovable dev</span>";
